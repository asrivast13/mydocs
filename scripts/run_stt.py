...
if __name__ == '__main__':
    supported_engines = ['vosk', 'w2v2']
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--model", type=str, required=True, help="Path to the model"
    )
    parser.add_argument(
        "--inpath", type=str, required=True, help="Input Path"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--framelen", type=int, required=False, default=30, help="Length of each frame of Audio in msecs [30]"
    )
    parser.add_argument(
        "--winlen", type=int, required=False, default=20, help="Number of frames in each window for segmentation [20]"
    )
    parser.add_argument(
        "--cpuonly", action='store_true', help="Disable GPU processing for W2V2 engine even if available (only used for testing throughput)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ctcbeta", type=float, required=False, default=1.0, help="W2V2 CTC Decoding parameter Beta: (default 1.0)"
    )
    parser.add_argument(
    ...
    HSSttModelFolder  = args.model
    VoskModelFolder   = args.model
    outFolder = "./"
    ...
    useLM = args.uselm if args.uselm is not None else False
    useSegmentsInVosk = True if ((args.engine == 'w2v2') or (args.segment is not None and args.segment == True)) else False
    ...
    for audioFile in audioFilesList:
        ...
        segments = [] if segmenter is None else segmenter.process(audio, 0.75, 0.5, 5, 15, frameDurationMs/1000)
