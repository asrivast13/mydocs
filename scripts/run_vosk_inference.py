...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--filepath", type=str, required=True, help="Path to file"
    )
    parser.add_argument(
        "--outfile", type=str, required=False, help="Output file"
    )
    parser.add_argument(
        "--segment", action='store_true', help="Should segmentation be run on this file"
    )
    ...
    audio_filename = args.filepath

    sessionId = os.path.basename(audio_filename).split('.')[0]
    outCTMfile = "%s.ctm" % sessionId

    if(args.outfile):
        outCTMfile = args.outfile
    ...
    if(args.segment):
        wf.close();
        segmenter = AudioSegmenter(2, 30, 20, sampleRate, 2)
        audio, sample_rate = read_wave(audio_filename)
        assert sample_rate == sampleRate
        segments = segmenter.process(audio, 0.75, 0.5, 5, 15, 30/1000)
